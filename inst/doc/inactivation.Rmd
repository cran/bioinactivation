...
The __bioinactivation__ package allows to predict the inactivation process of a population of microorganism microorganisms exposed to isothermal conditions (temperature remains constant with time) or non isothermal (temperature changes with time, dynmic conditions). Moreover, it can be used to adjust the parameters of different inactivation models to both isothermal and dynamic experimental data. All the functions contained in __bioinactivation__ are written in R.
...
Furthermore, two data sets mimicking the results of an isothermal inactivation experiment (`isothermal_inactivation`) and a dynamic experiment (`dynamic_inactivation`) are included in the package.
...
In this document, the functionalities of the __bioinactivation__ package are presented.
...
The first columns contains the output times.
...
The function `fit_inactivation_MCMC()` has nine input arguments:
...
The first eight are identical in contents and format to those defined for `fit_dynamic_inactivation()` and will not be described again here. The only addition is the `...` argument, which permits to include additional arguments for the `modMCMC()` function.

The same arguments used for the demonstration of `fit_dynamic_inactivation()` will be used in this example, with the addition of the `niter` argument, which sets the number of iterations of the MCMC.
...
*best_prediction* contains a list of class `SimulInactivation` with the prediction of the adjusted parameters. Finally, the data used for the fit is saved under _data_. The `FitInactivationMCMC` instance includes a S3 implementation of `plot()` for comparing the prediction against the data.
...
Mafart, P., Couvert, O., Gaillard, S., & Leguerinel, I. (2002). On calculating sterility in thermal preservation methods: Application of the Weibull frequency distribution model. _International Journal of Food Microbiology_, 72, 107–113.
