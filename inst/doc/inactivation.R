...
tibble(
  ...
  `k = 0.3` = log( 1 + exp(0.3*(Temperature - 60))),
  `k = 0.2` = log( 1 + exp(0.2*(Temperature - 60)))
  ) %>%
  pivot_longer(-Temperature, names_to = "k", values_to = "b") %>%
  ggplot() +
  geom_line(aes(x = Temperature, y = b, colour = k))
